...
public class SignupService {
	...
	public Response signup(@QueryParam("uuid") String uuid, @QueryParam("email") String email){
		...
		try {
			...
			String token = Util.randomString(4);
			...
			Util.sendEmail(email, subject, emailBody, name);
	...
	public Response registerPushToken(final PushRegistry registry){
		Response response = new Response();
		if(Util.isEmptyString(registry.getUserEmail()) || Util.isEmptyString(registry.getDeviceUuid()) || Util.isEmptyString(registry.getPuskToken())){
			response.setMessage("Invalid Input");
			return response;
		}
		try {
