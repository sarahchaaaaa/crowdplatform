...
public class LabelManagementService {
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public ArrayList<LabelSummary> getLabelingList(@PathParam("studyId") String studyId) {
		ArrayList<LabelSummary> labelingList = new ArrayList<>();

		try {
			Integer.parseInt(studyId);
			System.out.println("study id:" + studyId);
			connection = DatabaseUtil.connectToDatabase();
			String query = "select * from mcs.label_summary where study_id=" + studyId
			...
			while (resultSet.next()) {
				try {
					...
					labelSummary.setId(id);
					labelSummary.setStudyId(Integer.parseInt(studyId));
					labelSummary.setName(name);
					labelSummary.setDescription(description);
					labelSummary.setModificationTime(modificationTime);
					labelSummary.setModificationTimeZone(modificationTimeZone);
					labelSummary.setPublishedVersion(publishedVersion);
					labelSummary.setPublishTime(publishTime);
					labelSummary.setPublishTimeZone(publishTimeZone);
					labelSummary.setState(state);
					labelSummary.setResponseCount(responseCount);
					labelSummary.setStartTime(startTime);
					labelSummary.setStartTimeZone(startTimeZone);
					labelSummary.setEndTime(endTime);
					labelSummary.setEndTimeZone(endTimeZone);
					labelSummary.setSchedule(schedule);

					labelingList.add(labelSummary);
				} catch (Exception e) {
					// TODO: handle exception
				}
			}
